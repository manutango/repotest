...
SELECT
--top 100
	10 as id_escenario_version, -- PRESPUESTO - Final
	...
	PROD_PETROLEO_CIV * ROUND(((RAND(CAST( NEWID() AS varbinary ))*(2))-1),4) as PROD_PETROLEO_CIV,
	PROD_GAS_CIV * ROUND(((RAND(CAST( NEWID() AS varbinary ))*(2))-1),4) as PROD_GAS_CIV,
	PROD_AGUA_CIV * ROUND(((RAND(CAST( NEWID() AS varbinary ))*(2))-1),4) as PROD_AGUA_CIV,
	-1 as PRODUCCION_BASE,
	ID_POZO,
	PROD_BRUTA_CIV  * ROUND(((RAND(CAST( NEWID() AS varbinary ))*(2))-1),4) as PROD_BRUTA_CIV
FROM DW.STO.FT_PRODUCCION_OIL_GAS_PROD_1 A
where id_escenario_version = 1
;

-- Limpieza de registros PPTO
DELETE DW.STO.FT_PRODUCCION_OIL_GAS_PROD_1 
where id_escenario_version = 10
...
--Adicionalmente, agregar estas cuatro columnas que toman el mismo valor solo cuando es PPTO:
UPDATE F
SET 
	PROD_PETROLEO_CIV_PPTO = PROD_PETROLEO_CIV,
	PROD_GAS_CIV_PPTO = PROD_GAS_CIV, 
	PROD_AGUA_CIV_PPTO = PROD_AGUA_CIV,
	PROD_BRUTA_CIV_PPTO = PROD_BRUTA_CIV	
...
--top 100
	...
	id_campaña,
	'CARGA_PP_FICTICIO_PRODUCCION_INY' as id_proceso,	
	INYECCION_CIV * ROUND(((RAND(CAST( NEWID() AS varbinary ))*(2))-1),4) as INYECCION_CIV,
	-1 as PRODUCCION_BASE,
	ID_POZO
...
UPDATE F
SET 
	INYECCION_CIV_PPTO = INYECCION_CIV
...
/*
select count(*) from DW.STO.FT_PRODUCCION_OIL_GAS_PROD_1_FICTICIO
select * from DW.STO.FT_PRODUCCION_OIL_GAS_PROD_1 where id_escenario_version = 10
select * from DW.STO.FT_PRODUCCION_OIL_GAS_INY_2_FICTICIO
select * from DW.STO.FT_PRODUCCION_OIL_GAS_INY_2 where id_escenario_version = 10
*/
